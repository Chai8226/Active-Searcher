...
/**
 * @brief 简化updateGridData函数，只保留初始化分配部分
 */
void HGrid::initHgridAlloaction(const int& drone_id, vector<int>& grid_ids,
  ...
  grid1_->initGridAllocation(drone_id, grid_ids1);
  grid2_->initGridAllocation(drone_id, grid_ids2);
...
void HGrid::getCoverageCostMatrix(const vector<Eigen::Vector3d>& positions,
  ...
  for (int i = 0; i < drone_num; ++i) {
    for (int j = 0; j < grid_num; ++j) {
      double cost =
          getCoverageCostDroneToGrid(positions[i], grid_ids[j], first_ids[i], growth_vector);
      mat(1 + i, 1 + drone_num + j) = cost;
      mat(1 + drone_num + j, 1 + i) = 0;
    }
  }
...
/// @brief 如果grid_id与当前位置相邻，调用涉及growth_vector的代价计算
/// @param pos
/// @param grid_id
/// @param first
...
double HGrid::getCoverageCostDroneToGrid(const Eigen::Vector3d& pos, const int& grid_id,
    const vector<int>& first, const Eigen::Vector3d& growth_vector = Eigen::Vector3d(0, 0, 0)) {
  ...
  Eigen::Vector3d dir = Eigen::Vector3d(grid.center_ - cur_grid.center_);  // 两个网格中心的向量
  ...
  double growth_encouragement = 0.2 * growth_vector.dot(dir.normalized());  // 计算生长方向的代价
