...
UniformGrid::UniformGrid(
  ...
  double grid_size;
  nh.param("partitioning/grid_size", grid_size, 4.2);
 ...
 */
void UniformGrid::initGridAllocation(const int& drone_id, vector<int>& grid_ids){
  ...
  if (!initialized_) {
    ...
    // else
    //   grid_ids = {};
    ROS_WARN("Init one time grid allocation.");
    initialized_ = true;
  } 
}
