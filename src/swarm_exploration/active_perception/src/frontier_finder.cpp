...
// ================================== inspect ====================================
void FrontierFinder::getFrontierViewpoints(int ftr_id, vector<Viewpoint>& vps) {
...
void FrontierFinder::getInspectCostMatrix(const vector<Vector3d>& positions, const vector<Vector3d>& velocities, 
                                          const vector<double>& yaws, const vector<int>& ftr_ids, 
                                          const Eigen::Vector3d& island_bmin, const Eigen::Vector3d& island_bmax,
                                          Eigen::MatrixXd& mat) {
  const int ftr_num = ftr_ids.size();
  if (ftr_num == 0) {
    mat.resize(0, 0);
  ...
  const int dimen = 1 + ftr_num; // 1 (drone) + N (frontiers)
  mat.resize(dimen, dimen);

  // --- 1. 计算表面探索完成度 E_surf ---
  double E_surf = 0.0;
  // TODO: 后续可以实现更精细的 E_surf 计算，例如统计表面被观测到的体素比例。

  // --- 2. 建立 ftr_id 到矩阵索引的映射，并缓存视点信息 ---
  vector<list<Frontier>::iterator> ftr_iters;
  vector<Viewpoint> viewpoints;
  for (int id : ftr_ids) {
    for (auto it = frontiers_.begin(); it != frontiers_.end(); ++it) {
      if (it->id_ == id) {
        ftr_iters.push_back(it);
        viewpoints.push_back(it->viewpoints_.front()); // 使用每个前沿点的最佳视点
        break;
      }
    }
  }
  ...
  // --- 3. 计算表面惩罚项 C_surf ---
  vector<double> surface_penalties;
  const double alpha = 1.0; // 惩罚形状参数
  const double lambda = 2.0; // 惩罚权重

  for (const auto& vp : viewpoints) {
    // a. 计算 d_surf(h2): 视点到包围盒表面的最短距离
    double d_surf = std::numeric_limits<double>::max();
    for (int i = 0; i < 3; ++i) {
      d_surf = std::min(d_surf, std::abs(vp.pos_[i] - island_bmin[i]));
      d_surf = std::min(d_surf, std::abs(vp.pos_[i] - island_bmax[i]));
    }

    // b. 计算 S(h2)
    double S_h2 = 1.0 - exp(-alpha * d_surf);

    // c. 计算 C_surf(h2)
    double C_surf = lambda * (1.0 - E_surf) * S_h2;
    surface_penalties.push_back(C_surf);
  }

  // --- 4. 填充代价矩阵 ---
  const Vector3d& cur_pos = positions.front();
  const Vector3d& cur_vel = velocities.front();
  const double& cur_yaw = yaws.front();

  // a) 从无人机当前位置到各个前沿点
  for (int j = 0; j < ftr_num; ++j) {
    vector<Vector3d> path;
    double path_cost = ViewNode::computeCost(cur_pos, viewpoints[j].pos_, cur_yaw, viewpoints[j].yaw_, cur_vel, 0.0, path);
    mat(0, 1 + j) = path_cost + surface_penalties[j];
  }
  mat.col(0).setZero(); // 从任何点回到无人机当前位置的代价不考虑
  ...
  for (int i = 0; i < ftr_num; ++i) {
    for (int j = 0; j < ftr_num; ++j) {
      ...
      vector<Vector3d> path;
      double path_cost = ViewNode::computeCost(viewpoints[i].pos_, viewpoints[j].pos_, viewpoints[i].yaw_, viewpoints[j].yaw_, Vector3d::Zero(), 0.0, path);
      mat(1 + i, 1 + j) = path_cost + surface_penalties[j]; // 惩罚只与目的地有关
    }
  } 
}
