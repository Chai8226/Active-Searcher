...
const int BsplineOptimizer::NORMAL_PHASE =
    ...
    BsplineOptimizer::START | BsplineOptimizer::END; //| BsplineOptimizer::WAYPOINTS
...
void BsplineOptimizer::optimize(Eigen::MatrixXd& points, double& dt, const int& cost_function,
  ...
  // ---------------------------- 输入校验 ---------------------------- //
  // Validate control points matrix dimensions
  if (points.rows() < 4) {
    ROS_ERROR_STREAM("[BsplineOpt] Control points rows insufficient! Required >=4, actual=" << points.rows());
    return;
  }
  if (points.cols() != dim_) {
    ROS_ERROR_STREAM("[BsplineOpt] Control points columns mismatch! Required=" << dim_ << ", actual=" << points.cols());
    return;
  }
  
  // Verify initial states
  if (start_state_.empty()) {
    ROS_ERROR("[BsplineOpt] Initial state not configured!");
    return;
  }
  if (start_state_.size() < 3) {
    ROS_ERROR_STREAM("[BsplineOpt] Initial state requires at least 3 elements (pos+vel+acc), actual=" << start_state_.size());
    return;
  }
  
  // Validate guide path size
  if ((cost_function & GUIDE) && !guide_pts_.empty()) {
    int required_guide_size = points.rows() - 2 * order_;
    if (guide_pts_.size() < required_guide_size) {
      ROS_ERROR_STREAM("[BsplineOpt] Insufficient guide points! Required=" << required_guide_size 
                       << ", actual=" << guide_pts_.size());
      return;
    }
  }
  
  // Check waypoint indices validity
  for (size_t i = 0; i < waypt_idx_.size(); ++i) {
    int idx = waypt_idx_[i];
    if (idx < 0 || idx >= points.rows() - 2) {
      ROS_ERROR_STREAM("[BsplineOpt] Waypoint index out of bounds! idx=" << idx 
                       << ", valid range [0," << points.rows()-2 << ")");
      return;
    }
  }
  
  // Verify dynamic timing configuration
  if (dynamic_ && start_time_ < 0) {
    ROS_ERROR("[BsplineOpt] Dynamic mode enabled but start_time not set!");
    return;
  }
  
  // Validate parameter initialization
  if (dist0_ < 0 || swarm_safe_dist_ < 0) {
    ROS_ERROR("[BsplineOpt] Critical parameters not loaded! dist0=%f, swarm_safe_dist=%f", dist0_, swarm_safe_dist_);
    return;
  }
  
  // Check for invalid floating-point values
  for (int i = 0; i < points.rows(); ++i) {
    if (!points.row(i).allFinite()) {
      ROS_ERROR_STREAM("[BsplineOpt] Control points contain invalid values (NaN/Inf) at row=" << i);
      return;
    }
  }
  // ------------------------------------------------------------------ //
