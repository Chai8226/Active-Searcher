...
PlanningVisualization::PlanningVisualization(ros::NodeHandle& nh) {
  ...
  island_pub_ = node.advertise<visualization_msgs::Marker>("/planning_vis/island", 100);
  pubs_.push_back(island_pub_);
 ...
 * @brief 在可视化中绘制多个多边形。
 *
 * @brief 此函数接收一个多边形列表，其中每个多边形由一个二维顶点列表定义，
 * @brief 并在地图上将它们可视化为轮廓。每个多边形的 z 坐标都将设置为 0。
 * @brief by gemini 
 *
 * @param polygon_vertices_list 一个常量引用，指向包含多个 Eigen::Vector2d 向量的向量。每个内部向量代表一个单独多边形的顶点。
 * @param scale 标记的比例（虽然对于线条，这可能不会直接影响粗细）。
 * @param color 多边形的 RGBA 颜色（Eigen::Vector4d，元素范围为 [0, 1]）。
 * @param ns ROS 标记的命名空间。
 * @param pub_id `pubs_` 映射中用于发布标记的发布者的索引。
 */
void PlanningVisualization::drawMultiplePolygons(const vector<vector<Eigen::Vector2d>>& polygon_vertices_list, const double& scale,
                                                 const Eigen::Vector4d& color, const string& ns, const int& pub_id) {

  visualization_msgs::Marker clear_mk;
  fillBasicInfo(clear_mk, Eigen::Vector3d(scale, scale, scale), color, ns, 0, visualization_msgs::Marker::LINE_STRIP);
  clear_mk.action = visualization_msgs::Marker::DELETEALL; // 删除该 ns 下所有 Marker
  pubs_[pub_id].publish(clear_mk);

  for (size_t i = 0; i < polygon_vertices_list.size(); ++i) {
    if (polygon_vertices_list[i].size() < 3) {
      ROS_WARN("Cannot draw a polygon with less than 3 vertices.");
      continue;
    }

    // pub new marker
    visualization_msgs::Marker mk;
    fillBasicInfo(mk, Eigen::Vector3d(scale, 0, 0), color, ns, i, visualization_msgs::Marker::LINE_STRIP);
    mk.action = visualization_msgs::Marker::ADD;

    for (const auto& vertex : polygon_vertices_list[i]) {
    ...
    // Close the polygon
    if (!polygon_vertices_list[i].empty()) {
      geometry_msgs::Point first_p;
      first_p.x = polygon_vertices_list[i].front().x();
      first_p.y = polygon_vertices_list[i].front().y();
      first_p.z = 1.0; // 设置 z 坐标为 0
      mk.points.push_back(first_p);
    }
