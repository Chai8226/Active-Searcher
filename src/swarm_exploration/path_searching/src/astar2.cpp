    ...
    for (double dx = -resolution_; dx <= resolution_ + 1e-3; dx += resolution_)
      for (double dy = -resolution_; dy <= resolution_ + 1e-3; dy += resolution_)
        for (double dz = -resolution_; dz <= resolution_ + 1e-3; dz += resolution_) {
          ...
          nbr_pos = cur_pos + step; //向四周前进一个step能碰见的都算邻居
          // Check safety
          if (!edt_env_->sdf_map_->isInBox(nbr_pos)) {  //在地图之中
            // std::cout << "not in box" << std::endl;
            continue;
          }

          // if (edt_env_->sdf_map_->getInflateOccupancy(nbr_pos) == 1 ||
          //     (!optimistic && edt_env_->sdf_map_->getOccupancy(nbr_pos) == SDFMap::UNKNOWN))  //判断Occupancy
          //   continue;
