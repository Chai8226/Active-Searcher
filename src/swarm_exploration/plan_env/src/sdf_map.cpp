...
void SDFMap::inputPointCloud(const pcl::PointCloud<pcl::PointXYZ>& points, const int& point_num,
  ...
  for (int i = 0; i < point_num; ++i) {
    ...
    setCacheOccupancy(vox_adr, tmp_flag); //标记点云中这个点的占据情况
    ...
    if (md_->flag_rayend_[vox_adr] == md_->raycast_num_)  //每个被投射过的点会在这个函数的开头打上独一无二标记raycast_num_，这里防止他被多次投射
      continue;
    else
    ...
    caster_->nextId(idx);
    while (caster_->nextId(idx)) setCacheOccupancy(toAddress(idx), 0);  //在点云中的那个点和相机之间被遍历到的元素都被光线穿透，认为都是未占据的
  }
    ...
    for (int y = md_->local_bound_min_(1); y <= md_->local_bound_max_(1); ++y)
      for (int z = md_->local_bound_min_(2); z <= md_->local_bound_max_(2); ++z) {
        ...
        if (md_->occupancy_buffer_[id1] > mp_->min_occupancy_log_) {
          ...
          for (int inf_x = -inf_step; inf_x <= inf_step; ++inf_x) //膨胀
